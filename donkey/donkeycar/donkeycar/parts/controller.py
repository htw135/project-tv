...
class reader:
    ...
    def __init__(self, pi, a_gpio,t_gpio, weighting=0.0, poll_delay=0.0,
                 max_throttle=1.0,
                 steering_axis='x',
                 throttle_axis='y',
                 #throttle_axis='rz',
        ...
        self._high = None
        self._hight = None
        self._hight_tick = None
        
        pi.set_mode(a_gpio, pigpio.INPUT)
        pi.set_mode(t_gpio, pigpio.INPUT) 
        
        self._cba = pi.callback(a_gpio, pigpio.EITHER_EDGE, self._cbf)
        self._cbt = pi.callback(t_gpio, pigpio.EITHER_EDGE, self._cbfd)

        
    def _cbf(self, a_gpio, level, tick):

 #       print ("_cbf: ", a_gpio, "level:", level, "tick:", tick, "high_tick:", self._high_tick)
    ...
    def _cbfd(self, t_gpio, level, tickd):

        if level == 1:
 
            if self._hight_tick is not None:
                u = pigpio.tickDiff(self._hight_tick, tickd)
 
                if self._period is not None:
                    self._period = ((self._old * self._period) + (self._new * u)) 
                else:
                    self._period = u
 
            self._hight_tick = tickd

        if level == 0:

            if self._hight_tick is not None:
                u = pigpio.tickDiff(self._hight_tick, tickd)
                if self._hight is not None:
                    self._hight = ((self._old * self._hight) + (self._new * u))
                else:
                    self._hight = u
    ...
    def duty_cycle(self):  
        ...
        """
        if self._hight and self._high is not None:
            return 100.0 * self._hight / self._period and 100.0 * self._high / self._period
        else:
            return 0.0
    ...
    def init_read(self):
        ...
        '''
        try:
            self.reada = self.a_gpio
            self.readt = self.t_gpio
        except FileNotFoundError:
            self.reada = None
            self.readt = None
        return self.reada and self.readt is not None

    def update(self):
        while self.running and not self.init_read():
            time.sleep(5)

        while self.running:
            if self._high or self._hight is not None:
                self.angle = self._high - 1500
                self.throttle = self._hight - 1500

                self.angle = self.angle * 0.0025
                print("angle", self.angle)

                #this value is often reversed, with positive value when pulling down
                self.throttle = self.throttle * 0.0025
...
#         print("angle==", self.angle,"throttle==", self.throttle)
        return self.angle, self.throttle, self.mode, self.recording
        print('1111', self.throttle)
    ...
    def update(self):
        ...
        * cross = PS3 cross => decrease max throttle
        ...
        while self.running:
            ...
            if button == 'thumb' and button_state == 1:
                '''
                increase throttle scale
                '''
                #self.throttle_scale = round(min(0.0, self.throttle_scale + 0.05), 2)
                self.throttle_scale = round(max(1.0, self.throttle_scale + 0.25), 2)
                self.on_throttle_changes()
                print('throttle_scale:', self.throttle_scale)

            if button == 'thumb2' and button_state == 1:
                '''
                decrease throttle scale
                '''
                if self.throttle_scale > 1  : self.throttle_scale =  1
                if self.throttle_scale < -1 : self.throttle_scale = -1
                #self.throttle_scale = round(max(-1.0, self.throttle_scale - 0.05), 2)
                self.throttle_scale = round(min(-1.0, self.throttle_scale - 0.25), 2)
                self.on_throttle_changes()
                print('throttle_scale:', self.throttle_scale)
